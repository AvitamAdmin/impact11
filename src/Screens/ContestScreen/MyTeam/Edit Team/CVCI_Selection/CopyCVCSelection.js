...
const CopyCVCSelection = () => {
  ...
  const playerList = useSelector(state => state.fantasy.EditFinalPlayerSelected);
  ...
  const editTeamRecordId = useSelector(state => state.fantasy.editTeamRecordId);
    ...
    const fetchToken = async () => {
      try {
        ...
        if (token) {
          ...
          await fetchEditTeam(token, editTeamRecordId);
        } else {
  ...
  const fetchPlayerList = async (token, captain, ViceCaptain, impact) => {
    ...
    try {
      ...
      const editCaptain = filteredPlayers.filter(
        item => item.recordId === captain,
      );

      dispatch(setcaptain(editCaptain[0].recordId));
      dispatch(setCaptainImage(editCaptain[0].playerImagePath));
      dispatch(setcaptainTeam(editCaptain[0].teamDto?.shortName || 'N/A'));

      const editViceCaptain = filteredPlayers.filter(
        item => item.recordId === ViceCaptain,
      );


      dispatch(setViceCaptain(editViceCaptain[0].recordId));
      dispatch(setViceCaptainImage(editViceCaptain[0].playerImagePath));
      dispatch(
        setViceCaptainTeam(editViceCaptain[0].teamDto?.shortName || 'N/A'),
      );

      const editImpactPlayer = filteredPlayers.filter(
        item => item.recordId === impact,
      );

      dispatch(setimpactPlayer(editImpactPlayer[0].recordId));
      dispatch(setimpactPlayerImage(editImpactPlayer[0].playerImagePath));
      dispatch(
        setImpactPlayerTeam(editImpactPlayer[0].teamDto?.shortName || 'N/A'),
      );
    } catch (error) {
  ...
  const save = () => {
    ...
    // Dispatch player selection update
    dispatch(setplayerListwithStatus(playerList));

  };

  // ✅ useEffect will call handleSave only after PlayersforDto updates
  useEffect(() => {
  ...
  }, [PlayersforDto]); // ✅ Runs only when PlayersforDto updates

  const handleSave = async token => {


    // Step 1: Convert filter array to a map for fast lookup by playerId
    const filterMap = new Map();
    ToFilter.forEach(player => {
      filterMap.set(player.playerId, {
        captain: player.captain,
        viceCaptain: player.viceCaptain,
        impactPlayer: player.impactPlayer
      });
    });

    // Step 2: Compare each player in PlayersforDto with the one in filterMap
    let allMatch = true;

    for (const dto of PlayersforDto) {
      const match = filterMap.get(dto.playerId);

      if (!match) {
        // console.log(`Player with ID ${dto.playerId} not found in filter list.`);
        allMatch = false;
        continue;
      }

      if (
        match.captain !== dto.captain ||
        match.viceCaptain !== dto.viceCaptain ||
        match.impactPlayer !== dto.impactPlayer
      ) {
    ...
    if (allMatch) {
      showMessage({
        message: `Already this team exists`,
        type: 'danger',
        position: 'top',
        duration: 2000,
      });
      ...
      try {
        ...
        if (response.data.success === true) {
          navigation.reset({
            index: 0,
            routes: [{ name: 'MyTeam' }], // Clear the stack
          });
  ...
  const fetchEditTeam = async (token, editTeamRecordId) => {


    try {
      const body = {
        userTeamsDtoList: [{ recordId: editTeamRecordId }],
      };
      const headers = { Authorization: `Bearer ${token}` };
      const response = await axios.post(
      ...
      // console.log(
      //   'Edit team details:',
      //   response.data.userTeamsDtoList .flatMap(item => item.players)
      // );
      const captain = response.data.userTeamsDtoList
        .flatMap(item => item.players)
        .filter(id => id.captain == true);
      // console.log("Cap:",captain);

      const Vicecaptain = response.data.userTeamsDtoList
        .flatMap(item => item.players)
        .filter(id => id.viceCaptain == true);
      const impact = response.data.userTeamsDtoList
        .flatMap(item => item.players)
        .filter(id => id.impactPlayer == true);

      const c = captain[0].playerId;
      const vc = Vicecaptain[0].playerId;
      const i = impact[0].playerId;


      if (captain && ViceCaptain && impact) {
        await fetchPlayerList(token, c, vc, i);
      }
    } catch (error) {
      console.error('Error fetching edit teamaa:', error);
    }
  };
...
export default CopyCVCSelection;
