...
const EditCVCSelection = () => {
  ...
  // console.log('b4 editTeam:', editTeamRecordId);
  ...
  //     handleSave(Token);
  //     setIsSaved(true); // Prevents re-running the effect
  //   }
  // }, [PlayersforDto]);
  ...
  const fetchPlayerList = async (token, captain, ViceCaptain, impact) => {
    ...
    // console.log("tocheck:",playerList);
    ...
    try {
      ...
      // console.log("ooooooo:",filteredPlayers.map(
      //   item => item.recordId
      // ));
  ...
  const handleSave = async token => {
    ...
    if (!token) {
      console.error('Token is missing, API call skipped.');
      return;
    }
...
export default EditCVCSelection;
