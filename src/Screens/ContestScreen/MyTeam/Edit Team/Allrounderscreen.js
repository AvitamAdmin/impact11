...
const Allrounderscreen = ({allPlayers}) => {
  const dispatch = useDispatch();
  const selectedPlayer = useSelector(
    state => state.fantasy.EditFinalPlayerSelected,
  );
  const disable = useSelector(state => state.fantasy.DisableAllRounder);
      ...
      <ScrollView
        ...
        {Object.entries(allPlayers).map(([category, players]) => {
          const Allrounder = players.filter(player => {
            ...
            return (
              role.includes('all') || role.includes('rounder') || role === 'ar'
            );
          });

          if (Allrounder.length === 0) return null;

          return (
            <View key={category} style={{width: '100%', backgroundColor: ''}}>
              <View
                style={{
                  display: 'flex',
                  width: '100%',
                ...
                {Allrounder.map(player => (
                  <Pressable
                    ...
                    style={{
                      ...
                      borderRadius: 2,
                      borderColor: '#e6e6e6',
                      backgroundColor: selectedPlayer.includes(player.recordId)
                        ? '#555555'
                        : '#1A1A1A',
                      width: '100%',
                      alignItems: 'center',
                        ...
                        <View
                          ...
                          <View>
                            <Text
                              ...
                              {player.shortName || player.identiier || 'Player'}
                            </Text>
                          </View>
                          <View
                            style={{
