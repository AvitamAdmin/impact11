...
const Bowlerscreen = ({allPlayers}) => {
  const dispatch = useDispatch();
  const selectedPlayer = useSelector(
    state => state.fantasy.EditFinalPlayerSelected,
  );
  const disable = useSelector(state => state.fantasy.DisableBowler);
      ...
      <ScrollView
        ...
        {Object.entries(allPlayers).map(([category, players]) => {
          const Bowler = players.filter(player => {
            ...
            return (
              role.includes('Bow') || role.includes('ler') || role === 'bowl'
            );
          });

          if (Bowler.length === 0) return null;
