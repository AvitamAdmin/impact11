...
const UploadPanProof = ({ visible, onClose, setPanProof, onUploadSuccess }) => {
  ...
  const processImage = async (uri, name, type) => {
    try {
      ...
      const imageData = {
        uri,
        name: name || `bank_proof_${Date.now()}.${type.split('/')[1] || 'jpg'}`,
        type: type || 'image/jpeg',
        base64: `data:${type};base64,${base64Data}`,
      };

      setPanProof(imageData);
      onUploadSuccess();
    } catch (error) {
  ...
  //           type: document.type,
  //           base64: `data:${document.type};base64,${base64Data}`,
  //         });
  //         onUploadSuccess();
  //       } catch (err) {
  ...
  return (
    <Modal animationType="fade" transparent={true} visible={visible} onRequestClose={onClose}>
      ...
      >
        ...
        >
          <Text style={styles.modalTitle}>Upload Bank Proof</Text>
